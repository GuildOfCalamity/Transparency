...
public sealed class FocusBehavior : BehaviorBase<UIElement>
{
    ...
    static void OnTargetsPropertyChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
    {
    ...
    void OnEngagementTimerTick(object sender, object e)
    {
        ApplyFocus();
        Stop();
    }

    void OnControlLoaded(object sender, RoutedEventArgs e) => ApplyFocus();

    void OnTargetControlChanged(object? sender, EventArgs e) => ApplyFocus();
    ...
    void Stop(FocusTargetList? targets = null)
    {
        if (_timer != null)
        {
            _timer.Stop();
